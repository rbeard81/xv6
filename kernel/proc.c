...
//probably wasn't the right way to do it, but it works.
  for(;;){  
    sti();   
    acquire(&ptable.lock);
		...
		if (hat < winner) { 
			...
			for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
				if (p->state == RUNNABLE) {
					...
					//subtract tickets, the tickets that are higher have a higher chance of going subzero when selected from the winning value
				} 
	...
	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
		if (p->state == RUNNABLE){
			...
			for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
				...
				if(p->state != RUNNABLE || p->tickets < winner) {
					continue;
				}
				proc = p;
				switchuvm(p);
